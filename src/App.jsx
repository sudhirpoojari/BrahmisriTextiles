...
import sari1 from '../image/sari.jpg';
import sari2 from '../image/sari2.jpg';
import sari3 from '../image/sari3.jpg';
import sari4 from '../image/sari4.jpg';

import bc1 from '../image/bc1.jpg';
import bc2 from '../image/bc2.jpg';
import bc3 from '../image/bc3.jpg';
import bc5 from '../image/bc5.jpg';
import bc6 from '../image/bc6.jpg';

const slides = [
  { id: 1, url: sari1, title: 'Sari1' },
  { id: 2, url: sari2, title: 'Sari 2' },
  { id: 3, url: sari3, title: 'Sari 3' },
  { id: 4, url: sari4, title: 'Sari 4' },
];

const bestcollection = [
    { id: 1, url : bc1, title : 'Mysore Silks'   },
    { id: 2, url : bc2, title : 'Mysore Silks'   },
    { id: 3, url : bc3, title : 'Mysore Silks'   },
    { id: 4, url : bc5, title : 'Mysore Silks'   },
    { id: 5, url : bc6, title : 'Mysore Silks'   }, 
];
...
export default function App() {
  ...
  return (
    ...
    <div>
        ...
        <h1 className="text-6xl font-bold lg:[-webkit-text-stroke:1px_white]  text-amber-900 textoutli text-center  bg-linear-to-r from-cyan-200 to-blue-200">
             Brahmisri textiles
        </h1>
...
<div id="indicators-carousel" className="relative w-full" data-carousel="static">
    ...
    <div className="relative h-30 overflow-hidden rounded-base md:h-200">


        {slides.map((slide,index) => (
            <div key={slide.id} className={`duration-700 ease-in-out ${index === currentIndex ? 'block' : 'hidden'}`} data-carousel-item={index === currentIndex ? 'active' : ''}>
                <img src={slide.url} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt={slide.title} />
            </div>
        ))}
    ...
    <div className="absolute z-30 flex -translate-x-1/2 space-x-3 rtl:space-x-reverse bottom-5 left-1/2">
        {slides.map((_, index) => (
            <button key={index} onClick={() => setCurrentIndex(index)} type="button" className={`w-3 h-3 rounded-base ${index === currentIndex ? 'bg-white' : 'bg-white/50'}`} aria-current={index === currentIndex} aria-label={`Slide ${index + 1}`} data-carousel-slide-to={index}></button>
        ))}
    </div>
    ...
    <button onClick={nextSlide} type="button" className="absolute top-0 end-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next>
       ...
       <h1 className="text-4xl font-bold lg:[-webkit-text-stroke:1px_white]  text-emerald-700 textoutli text-center  p-1">
           Mysore Silks
        </h1>


    <div className="flex flex-row w-300 h-100 object-cover"  > 
   

        {bestcollection.map((item) => (
            <div key={item.id} className="basis-1/5 flex flex-col items-center gap-2">
                <img src={item.url} alt={item.title} className="w-full h-auto flex-shrink-0 border-2 p-1 rounded-3xl border-amber-50 outline-mist-50" />
                <p>{item.title}</p>
            </div>
        ))}
    ...
    <div className="pt-20">
      ...
      <footer className="relative w-full mb-60  bg-gray-400">

        <h2 className="text-4xl font-bold mb-4 text-center text-zinc-950" >Contact US</h2>
